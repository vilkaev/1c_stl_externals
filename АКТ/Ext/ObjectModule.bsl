...
Функция СведенияОВнешнейОбработке() Экспорт
	...
	ПараметрыРегистрации.Вставить("Наименование", "АКТ"); //имя под которым обработка будет зарегестрирована в справочнике внешних обработок
	...
	ПараметрыРегистрации.Вставить("Информация", "АКТ"); // т.е. это то, что будет видеть пользователь в информации к обработке
...
Функция ПечатьВнешнейПечатнойФормы(МассивОбъектов, ОбъектыПечати ) Экспорт
	...
	Если НЕ МассивОбъектов.Количество()=0 Тогда
		...
		ИначеЕсли ИННКонтрагента = ИннГАС Или ИННКонтрагента = ИннНГКМ Тогда 
...
#Область Получить_ТабДок
	...
	Функция ПолучитьТабДок_ГАС(СсылкаНаОбъект, ОбъектыПечати,ТабДок) Экспорт
		...
		ОбластьШапкаТабличнойЧасти =  Макет.ПолучитьОбласть("ОбластьШапкаТабличнойЧасти");
	...
	//	ТабДок.Вывести(ОбластьМакетаШапка);
		...
		Для Каждого Стр из Выборка Цикл
			...
			Если ИННКонтрагента = ИннНГКМ Тогда
				НаименованиеТовара = яОбщийМодуль.СформироватьСтрокуНаименованиеТовара(Стр, НастройкиПечати, "Акт", ЭтоПростой, СтруктураПечатиСтроки);
				ЕдИзм = "рейс";
				Колво = "1";
				Цена = Стр.Цена;
			Иначе
		...
		ОбластьПодвал.Параметры.СуммаПрописью = СуммаПрописью;
		ОбластьПодвал.Параметры.НаименованиеЗаказчика = СведенияОГрузополучателе.ПолноеНаименование;
		ОбластьПодвал.Параметры.НаименованиеИсполнителя = СведенияОПоставщике.ПолноеНаименование;
		//
...
Процедура Печать(МассивОбъектовНазначения, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода) Экспорт
	...
	Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "АКТ") Тогда 
		...
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(КоллекцияПечатныхФорм, "АКТ", "АКТ", ТабличныйДокумент);
	КонецЕсли;
